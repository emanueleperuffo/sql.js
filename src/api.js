...
Module['onRuntimeInitialized'] = function onRuntimeInitialized() {
  ...
  /**
   * @param {PointerBind} ptr
   * @param {number} pos
   */
  Statement.prototype.bindPointer = function bindPointer(ptr, pos) {
    if (pos == null) {
      pos = this.pos;
      this.pos += 1;
    }

    var ptrTypeBytes = intArrayFromString(ptr.ptrType);
    var ptrTypePtr = allocate(ptrTypeBytes, ALLOC_NORMAL);
    this.allocatedmem.push(ptrTypePtr);

    this.db.handleError(
      sqlite3_bind_pointer(this.stmt, pos, ptr.ptr, ptrTypePtr, NULL),
    );
  };
  ...
  class PointerBind {
    constructor(ptrType) {
      ...
      this.ptr = stackAlloc(4);
      setValue(this.ptr, 0, 'i32');
    }
  ...
  Statement.prototype.bindValue = function bindValue(val, pos) {
    ...
    if (val instanceof PointerBind) {
      return this.bindPointer(val, pos);
    }
  ...
  function Database(data) {
    ...
    _sqlite3_regexp_init(this.db, 0, 0);
  ...
  Database.prototype['updateHook'] = function updateHook(callback) {
    ...
    function wrappedCallback(
      ...
      switch (operationCode) {
        ...
        default:
          throw (
            'unknown operationCode in updateHook callback: ' + operationCode
          );
      }
  ...
  Database.prototype['create_fts5_tokenizer'] = function create_fts5_tokenizer(
    name,
    tokenize,
  ) {
    const api = this.fts5_api();

    api.createTokenizerV2(name, tokenize);
  };
